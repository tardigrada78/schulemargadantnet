 ...
 * @param {string} ollamaBaseUrl - Optionale Ollama-Basis-URL
 * @param {Array<{name:string,mediaType:string,base64:string}>} documents - Optionale PDF-Dokumente (nur Anthropic)
 * @returns {Promise<string>}  - Die Antwort als Text
 */
export async function callAI(prompt, providerModel, temperature = 0.6, maxTokens = 1000, ollamaBaseUrl = null, documents = []) {
    ...
    const content = documents.length
      ? [
          ...documents.map((d, i) => ({
            ...
            ...(i === documents.length - 1 ? { cache_control: { type: "ephemeral" } } : {}),
          })),
